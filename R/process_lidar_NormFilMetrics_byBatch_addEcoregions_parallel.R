...
# TEST
rdata.txt <- "/mnt/r/Mex_Lidar/Cartodata/Jalisco/LAS/T2/Tiles_30m_las_RDATA/Jalisco_rdataList_1.txt"
...
# Loop over rdata files in parallel
# ncores <- detectCores()
# 
# cl <- makeCluster(ncores)
# registerDoParallel(cl)
# foreach (r=(1:length(rdata.files)),
#          .packages = c("lidR","signal","raster","rgeos")) %dopar% {
...
lasfun <- function(r) {
  ...
  tryCatch({
    ...
    if (QA.flags == 'Y') {
      ...
      #get point density
      la <- lasarea(las2qa)
      ptden <- nrow(lasdata)/(la)
      if (ptden < min_ptden | la == 0) {log[,3] <- 1; log[,4] <- ptden}
    ...
    if (calc.mets == 'Y') {
      ...
      eco.crop <- crop(eco, extent(las2mets))
      eco.area <- gArea(eco.crop,byid=TRUE)
      eco.majority <- eco.crop[which(row.names(eco.crop@data) %in% (names(which.max(eco.area)[1]))),]
      
      eco_L1 <- eco.majority@data$LEVEL1
      # eco_NM_L1 <- eco.majority@data$NAME_L1
      eco_L2 <- eco.majority@data$LEVEL2
      # eco_NM_L2 <- eco.majority@data$NAME_L2
      eco_L3 <- eco.majority@data$LEVEL3
      # eco_NM_L3 <- eco.majority@data$NAME
      ...
      if (met.type =='fusion') {
        ...
        f.metrics$eco_L1 <- eco_L1
        # f.metrics$eco_NM_L1 <- eco_NM_L1
        f.metrics$eco_L2 <- eco_L2
        # f.metrics$eco_NM_L2 <- eco_NM_L2 
        f.metrics$eco_L3 <- eco_L3 
        # f.metrics$eco_NM_L3 <- eco_NM_L3
        ...
        prof.metrics$eco_L1 <- eco_L1
        # prof.metrics$eco_NM_L1 <- eco_NM_L1
        prof.metrics$eco_L2 <- eco_L2
        # prof.metrics$eco_NM_L2 <- eco_NM_L2 
        prof.metrics$eco_L3 <- eco_L3 
        # prof.metrics$eco_NM_L3 <- eco_NM_L3
        ...
        mets.final$eco_L1 <- eco_L1
        # mets.final$eco_NM_L1 <- eco_NM_L1
        mets.final$eco_L2 <- eco_L2
        # mets.final$eco_NM_L2 <- eco_NM_L2 
        mets.final$eco_L3 <- eco_L3 
        # mets.final$eco_NM_L3 <- eco_NM_L3
...
# Copy files from ramdisk to storage in one batch
