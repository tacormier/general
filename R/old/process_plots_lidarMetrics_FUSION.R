...
# path.master, lasfiles, minht, min_ptden, vegflag, binsize, profdir, 
# synthProf, saveprof, saveWave, out.txt, out.metrics, reform, wavepdf
...
outlasfil.dir <- paste0(dirname(path.master), "/norm_fil/")
dir.create(outlasfil.dir, showWarnings = F)
...
for (p in 1:length(lasfiles)) {
  ...
  #get point density
  ptden[p] <- nrow(lasdata)/(lasarea(lasnorm))
...
#   if (log[p,9] == 1) next()
  ...
  outlasfil <- paste0(outlasfil.dir, basename(lasfiles[p]))
...
#   system(txt2las.cmd)
  if (file.exists(outlasfil)) {
    lasfil.list[p] <- outlasfil
    tc.log$status[p] <- "filtered_las"
  } else {
